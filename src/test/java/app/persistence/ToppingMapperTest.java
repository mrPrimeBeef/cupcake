...
class ToppingMapperTest {
    ...
    @Test
    void getAllBToppings() throws DatabaseException {
        ArrayList<Topping> topping = new ArrayList<>();
        String sql = "select * from test.topping";
        ...
        assertEquals(topping.size(),9);
        assertEquals(topping.get(0).getToppingName(),"Chokolade");
        assertEquals(topping.get(1).getToppingName(),"Blåbær");
        assertEquals(topping.get(2).getToppingName(),"Hindbær");

        // will fail
//             assertEquals(topping.size(), 8);
//             assertEquals(topping.get(1).getBottomName(),"Blåbær1");
    }
